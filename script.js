...
document.addEventListener('DOMContentLoaded', function() {
  const titles = ["Data Scientist", "Machine Learning Engineer", "Business Analyst", "Data Analyst"];
  let currentIndex = 0;
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  flipButtons.forEach(button => {
    button.addEventListener('click', function() {
      const flipCard = this.closest('.about-containers').querySelector('.flip-card');
  ...
  const adjustTextSize = () => {
    ...
    descriptions.forEach(description => {
      ...
      while (description.scrollHeight > description.clientHeight && fontSize > 10) {
        fontSize -= 1;
